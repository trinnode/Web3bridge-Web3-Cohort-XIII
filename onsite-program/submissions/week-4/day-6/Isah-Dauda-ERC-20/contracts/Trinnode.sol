...
contract Trinnode{
    ...
    function initialize(string memory _name, string memory _symbol, uint8 _decimals, uint _initialSupply, uint _maxSupply) onlyOwner external {
        if (initialized) revert ErrorLib.ALREADY_INITIALIZED();
        if (_initialSupply > _maxSupply) revert ErrorLib.CAP_EXCEEDED();
        if (_initialSupply == 0 || _maxSupply ==0) revert ErrorLib.ZERO_AMOUNT();
    ...
    function transfer(address _to, uint _amount) external returns (bool){
        ...
        if (balanceOf[msg.sender] < _amount) revert ErrorLib.INSUFFICIENT_BALANCE();
        if (accountMetadata[msg.sender].blacklisted || accountMetadata[_to].blacklisted) revert ErrorLib.UNAUTHORIZED();
    ...
    function transferFrom(address _from, address _to, uint _amount) external returns (bool){
        ...
        if (allowance[_from][msg.sender] < _amount) revert ErrorLib.INSUFFICIENT_ALLOWANCE();
